...
ga4_session__contains__ga4_event AS (
    ...
    {% if is_incremental() %}
    WHERE
        ga4_session__contains__ga4_event.ga4_date_partition > DATE_SUB(DATE('{{ max_partition_date }}'), INTERVAL {{ env_var('DBT_PACKAGE_GA4__INTERVAL_INCREMENTAL') }} + 1 DAY)
        AND DATE(ga4_session__contains__ga4_event.ga4_session__contains__ga4_event__timestamp) > DATE_SUB(DATE('{{ max_partition_date }}'), INTERVAL {{ env_var('DBT_PACKAGE_GA4__INTERVAL_INCREMENTAL') }} DAY)
    {% endif %}
),
...
ga4_event__contains__ga4_item AS (
    ...
    {% if is_incremental() %}
    WHERE
        ga4_event__contains__ga4_item.ga4_date_partition > DATE_SUB(DATE('{{ max_partition_date }}'), INTERVAL {{ env_var('DBT_PACKAGE_GA4__INTERVAL_INCREMENTAL') }} + 1 DAY)
        AND DATE(ga4_event__contains__ga4_item.ga4_event__contains__ga4_item__timestamp) > DATE_SUB(DATE('{{ max_partition_date }}'), INTERVAL {{ env_var('DBT_PACKAGE_GA4__INTERVAL_INCREMENTAL') }} DAY)
    {% endif %}
),
...
SELECT * FROM final
    ...
    {% if is_incremental() %}
    WHERE
        DATE(final.ga4_timeline_timestamp) > DATE_SUB(DATE('{{ max_partition_date }}'), INTERVAL {{ env_var('DBT_PACKAGE_GA4__INTERVAL_INCREMENTAL') }} DAY)
    {% endif %}
