...
WITH t1 AS (
    SELECT
        ...
        events.device.mobile_marketing_name AS ga4_session_device_mobile_marketing_name
    FROM
...
t2 AS (
    ...
    WHERE
        ...
        AND t1.ga4_session_device_mobile_marketing_name IS NOT NULL
),
...
t3 AS (
    SELECT
        ...
        t2.ga4_session_device_mobile_marketing_name
    FROM
        t2
    WHERE
        t2.rn = 1
...
final AS (
    SELECT
        ...
        t3.ga4_session_device_mobile_marketing_name
    FROM
        t3
)
