...
WITH t1 AS (
    SELECT
        ...
        events.device.web_info.hostname AS ga4_session_device_web_info_hostname
    FROM
...
t2 AS (
    ...
    WHERE
        ...
        AND t1.ga4_session_device_web_info_hostname IS NOT NULL
),
...
t3 AS (
    SELECT
        ...
        t2.ga4_session_device_web_info_hostname
    FROM
        t2
    WHERE
        t2.rn = 1
...
final AS (
    SELECT
        ...
        t3.ga4_session_device_web_info_hostname
    FROM
        t3
)
