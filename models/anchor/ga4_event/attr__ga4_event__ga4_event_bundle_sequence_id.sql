...
t2 AS (
    SELECT
        ...
        ROW_NUMBER() OVER(PARTITION BY t1.ga4_event_id ORDER BY t1.ga4_event_bundle_sequence_id ASC) AS rn
    FROM
        t1
    WHERE
        t1.ga4_date_partition IS NOT NULL
        AND t1.ga4_event_id IS NOT NULL
        AND t1.ga4_event_bundle_sequence_id IS NOT NULL
),
...
t3 AS (
    SELECT
        ...
        t2.ga4_event_id,
        t2.ga4_event_bundle_sequence_id
    FROM
        t2
    WHERE
        t2.rn = 1
...
final AS (
    SELECT
        ...
        t3.ga4_event_id,
        t3.ga4_event_bundle_sequence_id
    FROM
        t3
)
