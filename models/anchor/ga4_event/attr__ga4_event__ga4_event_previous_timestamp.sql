...
WITH t1 AS (
    SELECT DISTINCT
        ...
        TIMESTAMP(DATETIME(TIMESTAMP_MICROS(events.event_previous_timestamp), '{{ env_var('DBT_PACKAGE_GA4__TIME_ZONE', '+00') }}')) AS ga4_event_previous_timestamp
    FROM
...
t2 AS (
    SELECT
        ...
        ROW_NUMBER() OVER(PARTITION BY t1.ga4_event_id ORDER BY t1.ga4_event_previous_timestamp ASC) AS rn
    FROM
        t1
    WHERE
        t1.ga4_date_partition IS NOT NULL
        AND t1.ga4_event_id IS NOT NULL
        AND t1.ga4_event_previous_timestamp IS NOT NULL
),
...
t3 AS (
    SELECT
        ...
        t2.ga4_event_id,
        t2.ga4_event_previous_timestamp
    FROM
        t2
    WHERE
        t2.rn = 1
...
final AS (
    SELECT
        ...
        t3.ga4_event_id,
        t3.ga4_event_previous_timestamp
    FROM
        t3
)
